...
export const AddLocation: React.FC = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    menu: [
      { id: '1', name: 'Cơm gà Hải Nam', price: '45.000đ', img: 'https://images.unsplash.com/photo-1598515214211-89d3c73ae83b?w=100&h=100&fit=crop' },
      { id: '2', name: 'Mì quảng đặc biệt', price: '35.000đ', img: 'https://images.unsplash.com/photo-1595147353130-9759e663da5e?w=100&h=100&fit=crop' }
    ]
  });
  ...
  const renderStep1 = () => (
    <div style={{ display: 'flex', gap: '48px' }}>
      <div style={{ flex: 1 }}>
        <Input label="Tên địa điểm" placeholder="Ví dụ: Khách sạn Marriott Hà Nội" value={formData.name} onChange={(e) => setFormData({...formData, name: (e.target as HTMLInputElement).value})} />
        <Input label="Địa chỉ chi tiết" placeholder="Số nhà, tên đường..." value={formData.address} onChange={(e) => setFormData({...formData, address: (e.target as HTMLInputElement).value})} />
        <div style={{ display: 'flex', gap: '16px', marginBottom: '24px' }}>
          ...
          <div style={{ flex: 1 }}><Input label="SĐT Liên hệ" placeholder="09xx xxx xxx" value={formData.phone} onChange={(e) => setFormData({...formData, phone: (e.target as HTMLInputElement).value})} style={{ marginBottom: 0 }} /></div>
        </div>
        <div>
      ...
      <div style={{ flex: 1 }}>
        ...
        <Input label="Giờ mở cửa" placeholder="Ví dụ: 08:00 - 22:00" icon={<Clock size={18} />} value={formData.openingHours} onChange={(e) => setFormData({...formData, openingHours: (e.target as HTMLInputElement).value})} />
      </div>
    </div>
  );
  ...
  const renderStep2 = () => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      <div style={{ background: '#F8FAFC80', padding: '24px', borderRadius: '24px', border: '1px solid #F1F5F9' }}>
        ...
        <div style={{ display: 'flex', gap: '16px', marginBottom: '24px' }}>
          <div style={{ flex: 1 }}><Input label="Tên dịch vụ" placeholder="VD: Giữ xe miễn phí" style={{ marginBottom: 0 }} /></div>
          <div style={{ flex: 1.5 }}><Input label="Mô tả (không bắt buộc)" placeholder="Nhập mô tả ngắn về dịch vụ" style={{ marginBottom: 0 }} /></div>
          <div style={{ display: 'flex', alignItems: 'flex-end' }}>
            <Button variant="outline" style={{ height: '52px', gap: '8px', padding: '0 24px', borderRadius: '12px', color: '#3b82f6', borderColor: '#3b82f6' }}>
              <Plus size={18} /> Thêm
            </Button>
          </div>
        </div>
      ...
      <div style={{ background: '#F8FAFC80', padding: '24px', borderRadius: '24px', border: '1px solid #F1F5F9' }}>
        ...
        <div style={{ display: 'flex', gap: '24px', marginBottom: '32px' }}>
          ...
          <div style={{ flex: 1 }}><Input label="Tên món ăn" placeholder="VD: Cơm Gà Hải Nam" style={{ marginBottom: 0 }} /></div>
          <div style={{ flex: 1 }}><Input label="Giá bán (VNĐ)" placeholder="0" rightIcon={<span>đ</span>} style={{ marginBottom: 0 }} /></div>
        </div>

        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '16px', marginBottom: '32px' }}>
          <Button style={{ padding: '12px 24px', borderRadius: '12px', fontSize: '14px', gap: '8px' }}><Plus size={18} /> Thêm vào danh sách</Button>
            ...
            {formData.menu.map(item => (
              <div key={item.id} style={{ display: 'flex', alignItems: 'center', gap: '16px', padding: '12px', background: 'white', border: '1px solid #E2E8F0', borderRadius: '16px', boxShadow: '0 2px 4px rgba(0,0,0,0.02)' }}>
                ...
                <div style={{ display: 'flex', gap: '12px', color: '#94a3b8' }}>
                  <Trash2 size={16} style={{ cursor: 'pointer' }} /><Edit2 size={16} style={{ cursor: 'pointer' }} />
                </div>
              </div>
            ))}
          </div>
  ...
  const renderStep3Initial = () => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '40px' }}>
      ...
      <div style={{ background: 'white', border: '1px solid #F1F5F9', borderRadius: '24px', padding: '40px' }}>
        ...
        <div onClick={() => setFileUploaded(true)} style={{ 
            height: '240px', 
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
        ...
        <div style={{ 
            ...
            <div style={{ width: '56px', height: '56px', borderRadius: '50%', background: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid #F1F5F9' }}>
            ...
            <div style={{ textAlign: 'center', maxWidth: '400px' }}>
                ...
                <p style={{ fontSize: '13px', lineHeight: '1.6' }}>Vui lòng tải tệp Excel lên và thực hiện ánh xạ các cột dữ liệu để xem bản xem trước tại đây.</p>
            </div>
        </div>
      </div>
    </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
        ...
        <div style={{ padding: '20px', background: '#F0FDF4', borderRadius: '16px', border: '1px solid #DCFCE7', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
            ...
            <button style={{ color: '#3b82f6', fontSize: '13px', fontWeight: '700', background: 'transparent', border: 'none', cursor: 'pointer' }}>Thay đổi tệp</button>
          </div>
        </div>
      </div>
            ...
            {[
              ...
              <tr key={idx} style={{ borderBottom: idx < 2 ? '1px solid #F1F5F9' : 'none' }}>
                <td style={{ padding: '20px 24px' }}>
                   <div style={{ display: 'flex', alignItems: 'center', gap: '12px', fontSize: '14px', fontWeight: '700', color: '#1e293b' }}>
                ...
                <td style={{ padding: '20px 24px' }}>
                   <div style={{ position: 'relative', width: '280px' }}>
                     <select style={{ width: '100%', padding: '10px 16px', borderRadius: '10px', border: '1px solid #E2E8F0', background: '#F8FAFC', outline: 'none', appearance: 'none', fontSize: '14px' }}>
                        <option>{row.mapping}</option>
                     </select>
                     <ChevronDown size={16} style={{ position: 'absolute', right: '12px', top: '50%', transform: 'translateY(-50%)', color: '#94a3b8' }} />
                   </div>
                </td>
                <td style={{ padding: '20px 24px' }}>
                   <span style={{ padding: '4px 12px', borderRadius: '6px', background: row.statusColor + '10', color: row.statusColor, fontSize: '11px', fontWeight: '700' }}>{row.status}</span>
                </td>
              </tr>
            ))}
          </tbody>
  ...
  return (
    <>
      <div style={{ maxWidth: step === 3 ? '1200px' : '1000px', margin: '0 auto', paddingBottom: step === 3 ? '120px' : '80px', paddingTop: '40px' }}>
        {step !== 3 && (
            <div style={{ marginBottom: '32px' }}>
                <h2 style={{ fontSize: '28px', fontWeight: '800', color: '#000000', marginBottom: '8px', fontFamily: "'Times New Roman', Times, serif" }}>Thêm địa điểm mới</h2>
          ...
          <div style={{ 
            ...
            {step === 3 ? (
                <div style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center', gap: '24px', width: '100%' }}>
                    ...
                    {fileUploaded && (
                        <button onClick={() => setFileUploaded(false)} style={{ background: 'transparent', border: 'none', color: '#64748b', fontSize: '14px', fontWeight: '700', cursor: 'pointer' }}>Tải lại file khác</button>
                    )}
                    {!fileUploaded && (
                ...
                <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '24px', width: '100%' }}>
                    {step === 1 ? (
                        <button onClick={() => navigate('/admin/locations')} style={{ background: 'transparent', color: '#64748b', fontSize: '14px', fontWeight: '700', padding: '12px 24px', cursor: 'pointer' }}>Hủy bỏ</button>
                    ) : (
