          ...
          <div style={{ display: 'flex', gap: '48px' }}>
            ...
            <div style={{ flex: 1.2 }}>
              ...
              <div style={{ display: 'flex', gap: '16px', marginBottom: '24px' }}>
                <div style={{ flex: 1 }}><Input label="Tỉnh/Thành phố" value={locationData.city} style={{ marginBottom: 0 }} /></div>
                <div style={{ flex: 1 }}>
              ...
              <div style={{ display: 'flex', gap: '16px', marginBottom: '24px' }}>
                <div style={{ flex: 1 }}><Input label="Giờ mở cửa" value={locationData.openTime} icon={<Clock size={16} />} style={{ marginBottom: 0 }} /></div>
                <div style={{ flex: 1 }}><Input label="Giờ đóng cửa" value={locationData.closeTime} icon={<Clock size={16} />} style={{ marginBottom: 0 }} /></div>
              </div>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ width: '100%', height: '240px', background: '#f8fafc', borderRadius: '16px', border: '1px solid #E2E8F0', overflow: 'hidden', position: 'relative', marginBottom: '24px' }}>
                <img src="https://images.unsplash.com/photo-1526778548025-fa2f459cd5c1?w=600&h=400&fit=crop" alt="Map" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
              ...
              <div style={{ display: 'flex', gap: '16px', marginBottom: '32px' }}>
                <div style={{ flex: 1 }}><Input label="Kinh độ (Latitude)" value={locationData.lat} style={{ marginBottom: 0 }} /></div>
                <div style={{ flex: 1 }}><Input label="Vĩ độ (Longitude)" value={locationData.lng} style={{ marginBottom: 0 }} /></div>
              </div>
            ...
            <div style={{ display: 'flex', gap: '16px', alignItems: 'center' }}>
                {showSearch && (
                    <div style={{ position: 'relative', width: '280px' }}>
                        ...
                        <input type="text" placeholder="Tìm kiếm món ăn..." style={{ width: '100%', padding: '10px 12px 10px 36px', borderRadius: '10px', border: '1px solid #F1F5F9', fontSize: '13px', outline: 'none' }} />
                    </div>
                )}
        ...
        {title === 'Dịch vụ tiện ích' ? (
             <div style={{ background: 'white', borderRadius: '20px', border: '1px solid #F1F5F9', padding: '24px', display: 'flex', gap: '16px', flexWrap: 'wrap' }}>
                {locationData.services.map(s => (
                    <div key={s.id} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 20px', background: '#EFF6FF', color: '#3b82f6', borderRadius: '16px', fontSize: '14px', fontWeight: '600' }}>
                        {s.icon} <span>{s.name}</span>
                    </div>
                ))}
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 20px', border: '1px solid #E2E8F0', borderStyle: 'dashed', color: '#94a3b8', borderRadius: '16px', fontSize: '14px', fontWeight: '600', cursor: 'pointer' }}>
            ...
            <div style={{ background: 'white', border: '1px solid #F1F5F9', borderRadius: '20px', overflow: 'hidden' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    ...
                    <tbody>
                        {locationData.menu.length > 0 ? locationData.menu.map((item, idx) => (
                            <tr key={idx} style={{ borderBottom: idx < locationData.menu.length - 1 ? '1px solid #F8FAFC' : 'none' }}>
                                <td style={{ padding: '16px 24px' }}>
                                   <img src={item.image} alt={item.name} style={{ width: '64px', height: '64px', borderRadius: '12px', objectFit: 'cover' }} />
                                </td>
                                <td style={{ padding: '16px 24px' }}>
                                ...
                                <td style={{ padding: '16px 24px' }}>
                                   <span style={{ padding: '4px 12px', borderRadius: '8px', background: '#F1F5F9', color: '#64748b', fontSize: '11px', fontWeight: '700' }}>{item.category}</span>
                                </td>
                                <td style={{ padding: '16px 24px', fontSize: '14px', fontWeight: '800', color: '#3b82f6' }}>{item.price}</td>
                                <td style={{ padding: '16px 24px' }}>
                                    <div style={{ width: '40px', height: '20px', background: '#3b82f6', borderRadius: '20px', position: 'relative', cursor: 'pointer' }}>
                            ...
                            <tr>
                                <td colSpan={6} style={{ padding: '32px', textAlign: 'center', color: '#94a3b8', fontSize: '14px' }}>Chưa có món ăn nào trong thực đơn</td>
                            </tr>
                        )}
                    </tbody>
                </table>
                <div style={{ padding: '16px 24px', borderTop: '1px solid #F1F5F9', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <div style={{ display: 'flex', gap: '8px' }}>
                        <button style={{ width: '28px', height: '28px', borderRadius: '6px', border: '1px solid #F1F5F9', background: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#E2E8F0' }}><ChevronLeft size={14}/></button>
                        <button style={{ width: '28px', height: '28px', borderRadius: '6px', background: '#3b82f6', color: 'white', border: 'none', fontSize: '12px', fontWeight: '700' }}>1</button>
                        <button style={{ width: '28px', height: '28px', borderRadius: '6px', border: '1px solid #F1F5F9', background: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#64748b' }}><ChevronRight size={14}/></button>
                    </div>
                </div>
            </div>
        )}
             ...
             <div key={rev.id} style={{ background: 'white', borderRadius: '24px', padding: '32px', border: '1px solid #F1F5F9' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '20px' }}>
                   ...
                   <div style={{ display: 'flex', gap: '4px', color: '#fbbf24' }}>
                      {[1,2,3,4,5].map(s => <Star key={s} size={16} fill={s <= rev.rating ? "#fbbf24" : "none"} color="#fbbf24" />)}
                   </div>
                </div>
                ...
                {rev.images && (
                    <div style={{ display: 'flex', gap: '12px', marginBottom: '24px' }}>
                        {rev.images.map((img, i) => (
                             <img key={i} src={img} alt="Review" style={{ width: '120px', height: '90px', borderRadius: '12px', objectFit: 'cover' }} />
                        ))}
                    </div>
                )}
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '32px' }}>
          <div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '4px 12px', background: locationData.statusColor + '10', borderRadius: '8px', color: locationData.statusColor, fontSize: '12px', fontWeight: '700' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
             ...
             <div 
               ...
               style={{ 
                 width: '44px', height: '22px', background: isActive ? '#3b82f6' : '#E2E8F0', borderRadius: '12px', position: 'relative', cursor: 'pointer', transition: '0.2s' 
               }}
             >
               <div style={{ position: 'absolute', left: isActive ? '24px' : '4px', top: '3px', width: '16px', height: '16px', background: 'white', borderRadius: '50%', transition: '0.2s' }}></div>
             </div>
          </div>
        </div>
